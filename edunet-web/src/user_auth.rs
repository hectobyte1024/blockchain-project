...
impl UserManager {
    ...
    pub async fn load_users_from_db(&self) -> Result<(), String> {
        ...
        for db_user in db_users {
            ...
            let user = User {
                ...
                created_at: db_user.created_at.map(|dt| DateTime::<Utc>::from_naive_utc_and_offset(dt, Utc)).unwrap_or_else(Utc::now),
                last_login: db_user.last_login.map(|dt| DateTime::<Utc>::from_naive_utc_and_offset(dt, Utc)),
                is_verified: db_user.is_active.unwrap_or(true),
            };
    ...
    pub async fn create_demo_users(&self) -> Result<(), String> {
        // PRODUCTION SECURITY: Strong random passwords
        // For deployment, change these or remove demo users entirely
        let alice_password = "EduNet2025!Alice#Secure";
        let bob_password = "EduNet2025!Bob#Secure";
        let carol_password = "EduNet2025!Carol#Secure";
        
        tracing::warn!("⚠️ PRODUCTION WARNING: Demo users enabled with default passwords!");
        tracing::warn!("   Alice password: {}", alice_password);
        tracing::warn!("   Bob password: {}", bob_password);
        tracing::warn!("   Carol password: {}", carol_password);
        tracing::warn!("   CHANGE THESE PASSWORDS IMMEDIATELY IN PRODUCTION!");
