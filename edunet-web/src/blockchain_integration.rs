...
use blockchain_core::{
    ...
    sync::{SyncEngine, SyncConfig},
    Hash256, Amount, Result as BlockchainResult,
};

// Pure Rust blockchain implementation
// use blockchain_core::mining::{MiningController, MiningConfig};
...
impl BlockchainBackend {
    pub async fn new(is_bootstrap: bool, server_address: Option<String>, database: Arc<Database>) -> anyhow::Result<Self> {
        ...
        // Initialize network with consensus for serving blockchain data
        ...
        tracing::info!("✅ Blockchain sync engine initialized");
        ...
        // Load existing blocks from database into the blockchain
        backend.load_blocks_from_database().await?;

        Ok(backend)
    }

    /// Load blocks from database into the actual blockchain
    async fn load_blocks_from_database(&self) -> anyhow::Result<()> {
        use sqlx::Row;
        
        // Query blocks from database ordered by height
        let query = "SELECT height, block_hash, prev_hash, merkle_root, timestamp, nonce, difficulty, block_data 
                     FROM blocks 
                     WHERE height > 0 
                     ORDER BY height ASC";
        ...
        if rows.is_empty() {
            tracing::info!("📦 No historical blocks to load from database");
            return Ok(());
        }

        tracing::info!("📦 Loading {} blocks from database into blockchain...", rows.len());
        ...
        for row in rows {
            ...
            let tx_query = "SELECT tx_hash, from_address, to_address, amount, fee, memo, timestamp 
                           FROM transactions 
                           WHERE block_height = ? 
                           ORDER BY tx_index";
            
            let tx_rows = sqlx::query(tx_query)
                .bind(height)
                .fetch_all(self.database.pool())
                .await?;

            // Store transactions in memory for API access
            let mut transactions = self.transactions.write().await;
            
            for tx_row in &tx_rows {
                let tx_hash: String = tx_row.get("tx_hash");
                let from_address: String = tx_row.get("from_address");
                let to_address: String = tx_row.get("to_address");
                let amount: i64 = tx_row.get("amount");
                let fee: i64 = tx_row.get("fee");
                let memo: Option<String> = tx_row.get("memo");
                let timestamp: i64 = tx_row.get("timestamp");

                let tx_history = TransactionHistory {
                    hash: tx_hash.clone(),
                    transaction_type: "confirmed".to_string(),
                    amount: amount as u64,
                    amount_edu: amount as f64 / 100_000_000_000.0,
                    ...
                    confirmations: 1,
                    fee: fee as u64,
                    size: memo.as_ref().map(|m| m.len()).unwrap_or(0),
                };

                transactions.insert(tx_hash, tx_history);
            }

            tracing::info!("  ✓ Block #{} loaded with {} transactions", height, tx_rows.len());
        }

        tracing::info!("✅ All historical blocks loaded into blockchain");
    ...
    /// Synchronize blockchain from network peers
    pub async fn sync_blockchain(&self) -> anyhow::Result<()> {
        ...
        tracing::info!("✅ Blockchain synchronization complete!");
        Ok(())
    }
    ...
    pub async fn is_synced(&self) -> bool {
        self.sync_engine.is_synced().await
    }
    ...
    pub async fn get_network_status(&self) -> anyhow::Result<serde_json::Value> {
        ...
        // Mining statistics (placeholder for now)
        let mining_stats = serde_json::json!({
            "hashrate": 0,
            "blocks_mined": 0,
            "is_mining": false
        });
        ...
        Ok(serde_json::json!({
            ...
            "is_mining": mining_stats["is_mining"].as_bool().unwrap_or(false),
            "blocks_mined": mining_stats["blocks_mined"].as_u64().unwrap_or(0),
            "hash_rate": mining_stats["hash_rate"].as_u64().unwrap_or(0),
