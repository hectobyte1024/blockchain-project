...
class MockStorage : public IBlockchainStorage {
public:
    bool store_utxo(const Hash256&, uint32_t, const UTXOEntry&) override { return true; }
    std::optional<UTXOEntry> get_utxo(const Hash256&, uint32_t) override { return std::nullopt; }
    bool remove_utxo(const Hash256&, uint32_t) override { return true; }
    std::vector<UTXOEntry> get_utxos_by_address(const std::vector<uint8_t>&) override { return {}; }
    bool store_block_metadata(const Hash256&, const BlockMetadata&) override { return true; }
    std::optional<BlockMetadata> get_block_metadata(const Hash256&) override { return std::nullopt; }
    bool store_transaction_metadata(const Hash256&, const TransactionMetadata&) override { return true; }
    std::optional<TransactionMetadata> get_transaction_metadata(const Hash256&) override { return std::nullopt; }
    std::vector<Hash256> get_block_hashes_by_height(uint32_t) override { return {}; }
    std::optional<Hash256> get_best_block_hash() override { return std::nullopt; }
    bool set_best_block_hash(const Hash256&) override { return true; }
    uint32_t get_chain_height() override { return 0; }
    bool reorg_chain(const std::vector<Hash256>&, const std::vector<Hash256>&) override { return true; }
};
...
void test_validator_management() {
    ...
    MockStorage storage;
    HybridConsensusEngine engine(&storage);
...
void test_consensus_validation() {
    std::cout << "Testing consensus validation..." << std::endl;
    
    MockStorage storage;
    HybridConsensusEngine engine(&storage);
    
    ConsensusState genesis_state;
    genesis_state.current_height = 500;
    genesis_state.current_difficulty = 0x1d00ffff; // Easy difficulty for testing
    
    assert(engine.initialize_genesis(genesis_state));
    
    // Test PoW validation
    std::string block_data = "test_block_data_for_mining";
    Hash256 empty_validator_id; // Indicates PoW block
    
    // This would require actual mining for a real test, so we'll just test the structure
    bool pow_validation = engine.validate_block(empty_validator_id, block_data, 12345, empty_validator_id);
    std::cout << "✓ PoW validation structure tested" << std::endl;
    
    // Test PoS validation with validator
    PublicKey pub_key;
    Hash256 validator_id = SHA256::hash(reinterpret_cast<const uint8_t*>("test_validator"), 14);
    assert(engine.add_validator(validator_id, pub_key, 5000000));
    
    bool pos_validation = engine.validate_block(validator_id, block_data, 0, validator_id);
    std::cout << "✓ PoS validation structure tested" << std::endl;
    
    std::cout << "Consensus validation tests passed!" << std::endl;
}
...
void test_hybrid_difficulty_adjustment() {
    ...
    assert(adjusted == current_difficulty); // Should remain the same
    std::cout << "✓ Perfect ratio maintains difficulty" << std::endl;
    ...
    // Should decrease difficulty to encourage PoW
    std::cout << "✓ Low PoW ratio adjustment tested" << std::endl;
